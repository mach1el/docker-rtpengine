...
RUN apt-get update && apt-get install -yqq --no-install-recommends git curl wget ca-certificates \
    gnupg2 dirmngr debhelper default-libmysqlclient-dev gperf iptables-dev libavcodec-dev libavfilter-dev \
    libavformat-dev libavutil-dev libbencode-perl libcrypt-openssl-rsa-perl libcrypt-rijndael-perl libhiredis-dev \
    libio-multiplex-perl libio-socket-inet6-perl libjson-glib-dev libdigest-crc-perl libdigest-hmac-perl libnet-interface-perl \
    libnet-interface-perl libssl-dev libsystemd-dev libxmlrpc-core-c3-dev libcurl4-openssl-dev libevent-dev libpcap0.8-dev markdown \
    unzip nfs-common dkms libspandsp-dev build-essential lsb-release \
